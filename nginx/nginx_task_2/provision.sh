...
sudo yum install -y -q gcc gcc-c++ git wget vim make zlib-devel libtool autoconf httpd-tools epel-release
...
cp -f /vagrant/{nginx.conf,backend.conf} ~/nginx/ 

sed -i "s/IPstringforSED/$(hostname -I | sed "s/ /\n/g" | grep 192.168.56)/g" ~/nginx/backend.conf
sed -i "s/stringforSED/$(hostname | sed "s/kazak-nginx//g")/g" ~/nginx/nginx.conf
